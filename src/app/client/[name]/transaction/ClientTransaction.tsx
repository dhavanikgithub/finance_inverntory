...
export default function ClientTransaction({ clients }: ClientTransactionProps) {
  ...
  useEffect(() => {
    ...
    // dispatch(fetchClientByName(name?.toString()!!));
    dispatch(fetchBanks());
    dispatch(fetchCards());
  }, [dispatch]);
    ...
    <Dashboard>
      <SectionHeader>
        ...
        <SectionHeaderRight>
          ...
          {isModalOpen != null && clients.length > 0 &&
            <TransactionModal
              clients={clients}
              cards={cards}
              banks={banks}
              ...
              isSelectedClient={true} // Default to first client if none selected
            />
          }
        </SectionHeaderRight>
      </SectionHeader>
